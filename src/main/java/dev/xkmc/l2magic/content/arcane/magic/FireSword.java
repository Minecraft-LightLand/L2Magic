...
public class FireSword extends Arcane {
	...
	public boolean activate(Player player, MagicData magic, ItemStack stack, @Nullable LivingEntity target) {
		...
		if (!w.isClientSide()) {
			...
			EffectUtil.addEffect(target, new MobEffectInstance(LCEffects.FLAME.get(), time, 1),
	...
	private void strike(Level w, Player player, LivingEntity target) {
		if (!w.isClientSide()) {
			EffectUtil.addEffect(target, new MobEffectInstance(LCEffects.FLAME.get(), time, 0),
					EffectUtil.AddReason.SKILL, player);
		}
	}
