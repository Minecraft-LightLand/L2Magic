...
public class EarthAxe extends Arcane {
	...
	public boolean activate(Player player, LLPlayerData magic, ItemStack stack, @Nullable LivingEntity target) {
		...
		if (!w.isClientSide()) {
			...
			EffectUtil.addEffect(target, new MobEffectInstance(LLEffects.HEAVY.get(), time, 1),
	...
	private void strike(Level w, Player player, LivingEntity target) {
		if (!w.isClientSide()) {
			EffectUtil.addEffect(target, new MobEffectInstance(LLEffects.HEAVY.get(), time, 0),
					EffectUtil.AddReason.SKILL, player);
